...
#include "usart_cmd.h"
/* ==== 改动 1/3 (control.c) ====
   原来这里是 LAB1_TEST_MODE / LAB1_PWM / LAB1_PULSE_TICKS 三个 #define
   和一个 Lab1_Count 计数器。改成串口命令之后就不需要了 ——
   现在用 MATLAB 发一句 M,2500,300 就等于原来的
   LAB1_PWM=2500 + LAB1_PULSE_TICKS=60,而且不用重新编译烧录。
   ============================== */
...
{    
	...
	{   
      ...
      /* ==== 改动 2/3 (control.c) ====
         原来这里是 #if LAB1_TEST_MODE 的整段开环测试代码。
         换成"听 MATLAB 指挥"的手动模式。原来的平衡算法一行没动,
         只是被包进下面那个 else 里。 ============================== */
      UartCmd_Tick();                    /* 串口命令的电机看门狗,5ms 一次 */

      if(manual_mode==1)                 /* PWM 由 MATLAB 直接给 */
      {
          /* R4-4: 编码器是相对的,每次上电 TIM4->CNT 都是 10000,
             所以 5900/9900 这两个绝对边界在标定之前毫无意义 ——
             刚上电读到的 10000 本身就已经"越界"了。
             因此边界只有在上位机发过 HOME(把小车放到导轨中点,
             固件把计数写成 7925)之后才生效,即 enc_homed==1。
             生效之后也只拦"继续往那一端走"的方向,反方向永远放行,
             这样撞到边还能开回来。HOME 之前 M 命令被限幅成点动。 */
          if(Flag_Stop==1)                                   /* 按键急停 */
          {
              UartCmd_Abort();
              Moto=0;
              Set_Pwm(0);
                   ...
                   ((manual_pwm > 0 && Encoder <= UARTCMD_ENC_MIN) ||
                    (manual_pwm < 0 && Encoder >= UARTCMD_ENC_MAX)) )
          {                                                  /* 滑块到边,且还在往边上开 */
              UartCmd_Abort();
              Moto=0;
              Set_Pwm(0);
...
{  
	 ...
	 /* ==== R4-5: 角度环加积分项 (Lab 4 Step 4) ====
	    原来这里是纯 PD。handout 要求学生观察 I 作用,所以补上积分。
	    积分式和 handout 写的一致: integral(k)=integral(k-1)+error(k)*Ts,
	    Ts 是 TIM1 中断周期 5 ms,所以积分量的单位是 "ADC计数 x 秒",
	    学生看到的数值有物理含义。

	    抗饱和(anti-windup)不是可选项:摆杆被拿起来或者撞到边的时候,
	    误差会持续存在,积分会一路涨到几万,等控制器恢复时电机会猛地满
	    输出。这里的做法是限制积分"贡献的 PWM",而不是限制积分本身 ——
	    这样上限和 Ki 取值无关,学生把 Ki 调大 10 倍也不会突然失控。
	    Ki=0 时直接清零积分状态,所以关掉 I 是立刻干净的。 */
	 if(Balance_KI > 0.0001f || Balance_KI < -0.0001f)
	 {
		 float ilim;
		 Balance_Integral += Bias * 0.005f;          /* Ts = 5 ms */
		 ilim = BALANCE_I_PWM_LIMIT / Balance_KI;
		 if(ilim < 0) ilim = -ilim;
		 if(Balance_Integral >  ilim) Balance_Integral =  ilim;
		 if(Balance_Integral < -ilim) Balance_Integral = -ilim;
	 }
	 else
	 {
		 Balance_Integral = 0;                       /* I 关闭 = 无状态 */
	 }

	 balance=-Balance_KP*Bias-D_Bias*Balance_KD-Balance_KI*Balance_Integral;   //===倾角控制PWM  PID控制
