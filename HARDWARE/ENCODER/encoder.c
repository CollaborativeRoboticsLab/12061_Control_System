...
/**************************************************************************
函数功能：写入编码器计数值（用于串口 HOME 命令标定零位）
入口参数：定时器、计数值
返回  值：无
**************************************************************************/
void Write_Encoder(u8 TIMX,int value)
{
	switch(TIMX)
	{
		case 2:  TIM2->CNT = (u16)value; break;
		case 3:  TIM3->CNT = (u16)value; break;
		case 4:  TIM4->CNT = (u16)value; break;
		default: break;
	}
}
